  ...
  function documentHtml(settings,items,media,baseUrl){
    ...
    for(let offset=0;offset<items.length;offset+=perPage){
      const cards=items.slice(offset,offset+perPage).map((ex,j)=>{
        ...
        const notes=[ex.clinicalV02?'画像は共通の動作例です。表示された実施側で行ってください。':'',m?.imageCaption,m?.caution,ex.diseaseNote,ex.clinicalV02?.constraints,ex.clinicalV02?.sportPlan,ex.clinicalV02?.restSeconds?`セット間休息：${ex.clinicalV02.restSeconds}秒`:'',...(ex.note||'').split('\n')].map(v=>(v||'').trim()).filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i);
        return `<article class="exercise"><h2>${offset+j+1}. ${esc(ex.name)}</h2><p class="dose">${Object.entries(labels).filter(([k])=>p[k]).map(([k,v])=>`${v}：${esc(p[k])}`).join(' ／ ')} ／ 実施日：${ex.scheduleMode==='flexible'?'曜日指定なし（指示された週の頻度で実施）':ex.dows?.length?ex.dows.map(d=>days[d]).join('・')+'曜日':'毎日'}</p><div class="exercise-body"><div class="picture">${m?`<img src="${esc(new URL(m.imagePath||'assets/exercises/'+m.image,baseUrl).href)}" alt="${esc(ex.name)}">`:'<p>イラストなし<br>院内で確認した動作で行ってください。</p>'}</div><div class="instructions">${ex.params?`<p><b>補足：</b>${esc(ex.params)}</p>`:''}${steps.length?`<ol>${steps.map(step=>`<li>${esc(step)}</li>`).join('')}</ol>`:'<p>運動方法は担当の理学療法士から説明を受けてください。</p>'}</div>${notes.length?`<p class="caution"><b>注意：</b>${notes.map(esc).join(' ／ ')}</p>`:''}</div></article>`;
      }).join('');
      pages.push(`<section class="sheet"><header><h1>ご自宅で行う運動</h1><div>${settings.patientName?`${esc(settings.patientName)} 様`:'お名前：________________'}${settings.chartId?`　患者ID：${esc(settings.chartId)}`:''}</div><div>開始日：${esc(settings.startDate||'未設定')}${settings.nextVisit?`　次回来院日：${esc(settings.nextVisit)}`:''}${settings.therapistName?`　担当：${esc(settings.therapistName)}`:''}　${pages.length+1} / ${Math.ceil(items.length/perPage)}ページ</div></header><main>${cards}</main><footer><p><b>痛み・しびれが増したら中止し、担当の理学療法士に負荷と再開方法を相談してください。</b>図の左右より、あなたへの指示を優先してください。</p>${settings.restartInstructions?`<p>再開の指示：${esc(settings.restartInstructions)}</p>`:''}<p>相談先：${esc(settings.consultContact||'担当の理学療法士・受診している医療機関')}</p></footer></section>`);
    }
    return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ホームエクササイズ 印刷</title><style>
      *{box-sizing:border-box}body{margin:0;background:#e8edef;color:#182b32;font-family:"Yu Gothic",Meiryo,sans-serif;font-size:10pt;line-height:1.4}p{margin:2mm 0;white-space:pre-wrap;overflow-wrap:anywhere}h1{font-size:16pt;margin:0 0 1mm}h2{font-size:12pt;margin:0 0 2mm;overflow-wrap:anywhere}header{height:22mm;font-size:10pt;border-bottom:0.4mm solid #3d666e}header div{overflow-wrap:anywhere}main{height:231mm;display:grid;grid-template-rows:repeat(3,77mm)}footer{height:24mm;border-top:0.4mm solid #3d666e;font-size:9pt;padding-top:1mm}footer p{margin:1mm 0}.sheet{width:190mm;height:277mm;margin:10mm auto;background:white;box-shadow:0 2px 12px #0002}.exercise{height:77mm;padding:2mm 0;border-bottom:0.2mm solid #b4c2c6;break-inside:avoid}.exercise:last-child{border-bottom:0}.exercise-body{display:flow-root}.picture{float:left;width:57mm;margin:0 4mm 1mm 0}.picture img{display:block;width:57mm;height:38mm;object-fit:contain}.caption{font-size:9pt}.instructions{display:contents}.dose{font-weight:bold;margin-top:0}.instructions ol{display:flow-root;margin:1mm 0;padding-left:5mm}.instructions li{padding-bottom:0.5mm;overflow-wrap:anywhere}.caution{font-size:10pt}.toolbar{padding:16px;background:white;position:sticky;top:0;z-index:2;border-bottom:1px solid #ccc}.toolbar button{font:inherit;padding:10px 20px;margin-right:12px}.toolbar p{margin:8px 0}.error{color:#a12525;font-weight:bold}
