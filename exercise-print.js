  ...
  function documentHtml(settings,items,media,baseUrl){
    ...
    for(let offset=0;offset<items.length;offset+=perPage){
      const cards=items.slice(offset,offset+perPage).map((ex,j)=>{
        ...
        const notes=[m?.imageCaption,m?.caution,ex.diseaseNote,ex.clinicalV02?.constraints,ex.clinicalV02?.sportPlan,ex.clinicalV02?.restSeconds?`セット間休息：${ex.clinicalV02.restSeconds}秒`:'',ex.clinicalV02?`処方版：${ex.clinicalV02.revision}／PT確認日：${ex.clinicalV02.patient.date}`:'',...(ex.note||'').split('\n')].map(v=>(v||'').trim()).filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i);
        return `<article class="exercise"><h2>${offset+j+1}. ${esc(ex.name)}</h2><p class="dose">${Object.entries(labels).filter(([k])=>p[k]).map(([k,v])=>`${v}：${esc(p[k])}`).join(' ／ ')} ／ 実施日：${ex.dows?.length?ex.dows.map(d=>days[d]).join('・')+'曜日':'毎日'}</p><div class="exercise-body"><div class="picture">${m?`<img src="${esc(new URL(m.imagePath||'assets/exercises/'+m.image,baseUrl).href)}" alt="${esc(ex.name)}">`:'<p>イラストなし<br>院内で確認した動作で行ってください。</p>'}</div><div class="instructions">${ex.params?`<p><b>補足：</b>${esc(ex.params)}</p>`:''}${m?.steps?.length?`<ol>${m.steps.map(step=>`<li>${esc(step)}</li>`).join('')}</ol>`:'<p>運動方法は担当の理学療法士から説明を受けてください。</p>'}</div>${notes.length?`<p class="caution"><b>注意：</b>${notes.map(esc).join(' ／ ')}</p>`:''}</div></article>`;
      }).join('');
