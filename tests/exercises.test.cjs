...
test('each clinical library has ten unique illustrated exercises',()=>{
  ...
  for(const region of ['shoulder','lowback','knee'])assert.equal(entries.filter(([,e])=>e.region===region).length,10);
  assert.equal(new Set(entries.map(([,e])=>e.image)).size,30);
...
test('offline asset list includes all thirty illustration files',()=>{
  ...
  assert.equal(sw.files.filter(f=>f.startsWith('./assets/exercises/')).length,30);
});
