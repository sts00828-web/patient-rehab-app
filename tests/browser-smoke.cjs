...
async function main(){
  ...
  await evaluate("for(const key of ['neck-rotation','scapular-setting','walking']){const i=TEMPLATES.cervicalSpondylosis.menu.findIndex(ex=>ex.exerciseKey===key);$('pick-'+i).checked=true;$('dose-'+i).value='PT確認用：5回または指定時間';}applySelectedTemplate();showShareQR();window.neckShareUrl=buildShareUrl();");
  await check('neck QR contains selected shared IDs and patient restrictions',"(()=>{const p=JSON.parse(LZString.decompressFromEncodedURIComponent(decodeURIComponent(neckShareUrl.split('#d=')[1])));return p.patientId==='another_patient'&&p.menu.length===3&&p.menu.some(ex=>ex.exerciseKey==='walking')&&p.menu.every(ex=>ex.note.includes(DISEASE_LIBRARY.cervicalSpondylosis.prescriptionNote))&&!('diagnosis' in p)&&!!document.querySelector('#qrBox svg')})()");
  ...
  await check('neck reimport displays three prescribed exercises with restrictions and no fabricated records',"neckImportConfirmed&&S.patientId==='another_patient'&&Object.keys(L).length===0&&document.querySelectorAll('#today-content .exercise-card').length===3&&S.menu.every(ex=>ex.note.includes(DISEASE_LIBRARY.cervicalSpondylosis.prescriptionNote))&&$('today-content').textContent.includes('手の不器用さ')");
  ...
  await evaluate("for(const key of ['shoulder-isometric-external','shoulder-band-external','wall-push-up']){const i=TEMPLATES.shoulderImpingement.menu.findIndex(ex=>ex.exerciseKey===key);$('pick-'+i).checked=true;$('dose-'+i).value='PT確認用：軽い力で5回';}updateTemplateSelection();");
  ...
  await evaluate("applySelectedTemplate();showShareQR();window.impingementShareUrl=buildShareUrl();");
  await check('shoulder restrictions survive QR and current prescription shows loading warning',"(()=>{const p=JSON.parse(LZString.decompressFromEncodedURIComponent(decodeURIComponent(impingementShareUrl.split('#d=')[1])));return p.menu.length===3&&p.menu.every(ex=>ex.note.includes(DISEASE_LIBRARY.shoulderImpingement.prescriptionNote))&&$('therapist-content').textContent.includes('肩を外へ回す筋肉への負荷')&&!!document.querySelector('#qrBox svg')})()");
  ...
  await check('shoulder patient view has three exercises and imported restrictions without changing patient identity',"impingementImportConfirmed&&S.patientId==='another_patient'&&Object.keys(L).length===0&&document.querySelectorAll('#today-content .exercise-card').length===3&&S.menu.every(ex=>ex.note.includes(DISEASE_LIBRARY.shoulderImpingement.prescriptionNote))&&$('today-content').textContent.includes('術後・脱臼後の指示とは別')");
  for(const disease of ['rotatorCuffTear','cervicalDiscHerniation','anteriorShoulderDislocation','slapLesion']){
    ...
    await check(disease+' import shows three exercises and patient restrictions without making records',"reviewDiseaseImportConfirmed&&S.patientId==='another_patient'&&Object.keys(L).length===0&&document.querySelectorAll('#today-content .exercise-card').length===3&&JSON.stringify(S.menu.map(ex=>ex.exerciseKey))===JSON.stringify(reviewSharedKeys)&&S.menu.every((ex,i)=>ex.note.includes(DISEASE_LIBRARY[reviewDisease].prescriptionNote)&&mediaFor(ex).image===reviewSharedImages[i])&&$('today-content').textContent.includes(DISEASE_LIBRARY[reviewDisease].prescriptionNote)");
  }
