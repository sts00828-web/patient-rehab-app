...
async function main(){
  ...
  await check('neck reimport displays one of three prescribed exercises with restrictions and no fabricated records',"neckImportConfirmed&&S.patientId==='another_patient'&&Object.keys(L).length===0&&document.querySelectorAll('#today-content .exercise-card').length===1&&$('today-content').textContent.includes('運動 1 / 3')&&S.menu.every(ex=>ex.diseaseNote.includes(DISEASE_LIBRARY.cervicalSpondylosis.prescriptionNote))&&$('today-content').textContent.includes('手の不器用さ')");
  ...
  await check('shoulder patient view has a three-item carousel and imported restrictions without changing patient identity',"impingementImportConfirmed&&S.patientId==='another_patient'&&Object.keys(L).length===0&&document.querySelectorAll('#today-content .exercise-card').length===1&&$('today-content').textContent.includes('運動 1 / 3')&&S.menu.every(ex=>ex.diseaseNote.includes(DISEASE_LIBRARY.shoulderImpingement.prescriptionNote))&&$('today-content').textContent.includes('術後・脱臼後の指示とは別')");
  for(const disease of ['rotatorCuffTear','cervicalDiscHerniation','anteriorShoulderDislocation','slapLesion','lumbarSpinalStenosis','lumbarDiscHerniation','ankleSprain','meniscalInjury']){
    ...
    await check(disease+' import shows a three-item carousel and patient restrictions without making records',"reviewDiseaseImportConfirmed&&S.patientId==='another_patient'&&Object.keys(L).length===0&&document.querySelectorAll('#today-content .exercise-card').length===1&&$('today-content').textContent.includes('運動 1 / 3')&&JSON.stringify(S.menu.map(ex=>ex.exerciseKey))===JSON.stringify(reviewSharedKeys)&&S.menu.every((ex,i)=>ex.diseaseNote.includes(DISEASE_LIBRARY[reviewDisease].prescriptionNote)&&mediaFor(ex).image===reviewSharedImages[i])&&$('today-content').textContent.includes(patientCopy(DISEASE_LIBRARY[reviewDisease].prescriptionNote))");
  }
