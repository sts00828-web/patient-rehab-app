...
async function main(){
  ...
  await evaluate("window.confirm=()=>true;window.prompt=()=> 'テスト患者';window.alert=message=>{window.lastAlert=message};$('pin-input').value='1234';checkPin();updS('patientName','テスト患者');applyTemplate('shoulder');");
  await send('Emulation.setDeviceMetricsOverride',{width:320,height:700,deviceScaleFactor:1,mobile:true});
  await evaluate("$('chooseTemplate').remove();$('next-visit').closest('details').open=true;$('next-visit').value='2026-10-08';document.documentElement.style.setProperty('--top-inset','47px');$('hdr-name').textContent='表示名が長い患者さんのスマホ表示確認';");
  ...
  await check('legacy athlete API remains read-compatible without a second disease catalog',"(()=>{const before=JSON.stringify(S);applyTemplate('spondylolysisProtection');const good=document.querySelectorAll('#chooseTemplate .template-ex').length===4&&!$('pick-0').checked&&$('dose-0-load').value==='';closeModal('chooseTemplate');return good&&JSON.stringify(S)===before&&!document.querySelector('#exercise-catalog')})()");
  ...
  await check('athlete stages and volume presets preserve side, reset load and leave patient data untouched',"(()=>{const before=JSON.stringify(S);applyTemplate('spondylolysisReload');const i=getAllTemplates().spondylolysisReload.menu.findIndex(e=>e.exerciseKey==='single-leg-plank');$('pick-'+i).click();setQuickSide(i,'右');$('dose-'+i+'-load').value='重りなし';applyAthleteDose(i,3);const good=$('dose-'+i+'-sets').value==='3セット'&&$('dose-'+i+'-hold').value==='30秒'&&$('dose-'+i+'-side').value==='右'&&$('dose-'+i+'-load').value===''&&readPrescription('dose-'+i).scheduleMode==='flexible'&&readPrescription('dose-'+i).prescription.frequency.includes('週3日');$('template-athlete-level').value='2';filterTemplateExercises();const hidden=$('pick-'+i).closest('.template-ex').hidden;showPrescriptionErrors('dose-'+i,'テスト');const shown=!$('pick-'+i).closest('.template-ex').hidden&&document.activeElement.id==='dose-'+i+'-load-summary';closeModal('chooseTemplate');applyTemplate('spondylolysisProtection');const protectedMode=!document.querySelector('#chooseTemplate .athlete-dose');closeModal('chooseTemplate');return good&&hidden&&shown&&protectedMode&&JSON.stringify(S)===before})()");
  ...
  await check('built-in doses prefill drafts without choosing a patient side or confirming a prescription',"(()=>{const d=PRESCRIPTION_DEFAULTS.pendulum.prescription;return Object.keys(PRESCRIPTION_DEFAULTS).length===58&&$('dose-0-repetitions').value===d.repetitions&&$('dose-0-hold').value===d.hold&&$('dose-0-side').value===''&&!$('dose-0-schedule-confirmed').checked&&S.menu.length===0})()");
  ...
  await check('clinician defaults persist without prescribing automatically and require reconfirmation',`(()=>{
    fillTestPrescription('dose-0');$('dose-0-frequency').value='1日1回・週1日';savePrescriptionDefault('dose-0');
    ...
    const ok=saved.menu[0].prescription.side==='両側'&&saved.menu[0].scheduleConfirmed===false&&S.menu.length===0&&!(key in getAllTemplates())&&$('dose-0-side').value==='両側'&&!$('dose-0-day-1')&&$('dose-0-frequency').value==='1日1回・週1日'&&!$('dose-0-schedule-confirmed').checked;
  ...
  await evaluate("closeTherapist();");
  await check('pain is required on its own screen before exercises are shown',"!!document.querySelector('.pain-gate')&&!$('today-start')&&getLog(todayKey()).vas===null&&document.querySelectorAll('.pain-gate .pain-choice').length===11");
  await evaluate("document.querySelector('.pain-gate .pain-choice').click()");
  ...
  await send('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
  await check('clearing required pain returns to the pain-only screen',"(()=>{clearPain();return !!document.querySelector('.pain-gate')&&!$('today-start')&&getLog(todayKey()).vas===null})()");
  await evaluate("saveVas(0)");
  await check('partial and pain-related rest complete the day without claiming all were done',"(()=>{setExerciseStatus(0,'partial');setExerciseStatus(1,'pain');setExerciseStatus(2,'forgot');const text=$('today-content').textContent;return text.includes('今日の運動は終了です')&&text.includes('できなかった運動も')&&getCompletion(todayKey(),new Date().getDay()).done===0})()");
  await evaluate("patientSession=null;openPatientSession(0)");
  await check('completed day is also clear inside the exercise screen',"$('patientSession').textContent.includes('今日の運動はすべて記録できました')&&document.querySelector('#patientSession .session-actions .btn-pri').textContent.includes('今日の運動を終了')");
  await evaluate("closeModal('patientSession')");
  ...
  await check('last daily action scrolls above bottom navigation',"(async()=>{document.documentElement.style.scrollBehavior='auto';window.scrollTo(0,document.documentElement.scrollHeight);await new Promise(r=>requestAnimationFrame(r));const buttons=[...$('today-content').querySelectorAll('button')].filter(b=>b.checkVisibility()),last=buttons[buttons.length-1];return last.getBoundingClientRect().bottom<=document.querySelector('.nav').getBoundingClientRect().top-12})()");
  ...
  await evaluate('showExercise(0)');await delay(150);shot=await send('Page.captureScreenshot',{format:'png'});fs.writeFileSync(path.join(art,'guide-mobile.png'),Buffer.from(shot.data,'base64'));await evaluate("closeModal('patientSession')");
  ...
  await evaluate("window.beforeImmediate=C.clone(packState());$('pin-input').value='1234';checkPin();commitMenu([...S.menu,...[4,5,6,7].map(i=>({id:'added_'+i,name:'追加種目'+i,params:'5回',dows:[]}))]);renderToday();");
  ...
  await check('same-day action exposes seven in the carousel and retains existing records',"document.querySelectorAll('#today-content .exercise-card').length===1&&$('today-content').textContent.includes('運動 1 / 7')&&getCompletion(todayKey(),new Date().getDay()).done===3&&L[todayKey()].vas===beforeImmediate.logs[todayKey()].vas&&L[todayKey()].note===beforeImmediate.logs[todayKey()].note");
  await evaluate("loadState();renderToday();calSelectedKey=todayKey();renderCalendar();");
  await check('seven carousel exercises and previous records survive reload',"document.querySelectorAll('#today-content .exercise-card').length===1&&$('today-content').textContent.includes('運動 1 / 7')&&L[todayKey()].menuRevisions[0].menuSnapshot.length===3&&$('cal-content').textContent.includes('当日の変更前の記録')");
  ...
  await evaluate("$('pin-input').value='1234';checkPin();commitMenu([...S.menu,{id:'quota_extra',name:'保存失敗用',params:'5回',dows:[]}]);window.beforeFailedImmediate=JSON.stringify(packState());");
  ...
  await evaluate("$('pin-input').value='1234';checkPin();applyTemplate('lowback');");
  ...
  await evaluate("$('pin-input').value='1234';checkPin();window.menuBeforeElbow=JSON.stringify(S.menu);applyTemplate('tennisElbow');");
  ...
  await check('neck reimport requires pain without fabricating exercise records',"neckImportConfirmed&&S.patientId==='another_patient'&&Object.keys(L).length===0&&!!document.querySelector('.pain-gate')&&!document.querySelector('.exercise-card')");
  await evaluate("saveVas(0)");
  await check('neck reimport displays one of three prescribed exercises with restrictions',"document.querySelectorAll('#today-content .exercise-card').length===1&&$('today-content').textContent.includes('運動 1 / 3')&&S.menu.every(ex=>ex.diseaseNote.includes(DISEASE_LIBRARY.cervicalSpondylosis.prescriptionNote))&&$('today-content').textContent.includes('手の不器用さ')");
  await evaluate("$('pin-input').value='1234';checkPin();window.menuBeforeImpingement=JSON.stringify(S.menu);applyTemplate('shoulderImpingement');");
  ...
  await evaluate("closeQR();closeTherapist();L={};persist();window.impingementImportConfirmed=false;window.confirm=()=>{window.impingementImportConfirmed=true;return true};manualImport(impingementShareUrl);window.confirm=()=>true;");
  await check('shoulder patient view requires pain before its imported exercises',"impingementImportConfirmed&&S.patientId==='another_patient'&&Object.keys(L).length===0&&!!document.querySelector('.pain-gate')");
  await evaluate("saveVas(0)");
  await check('shoulder patient view has a three-item carousel and imported restrictions without changing patient identity',"document.querySelectorAll('#today-content .exercise-card').length===1&&$('today-content').textContent.includes('運動 1 / 3')&&S.menu.every(ex=>ex.diseaseNote.includes(DISEASE_LIBRARY.shoulderImpingement.prescriptionNote))&&$('today-content').textContent.includes('術後・脱臼後の指示とは別')");
  for(const disease of ['rotatorCuffTear','cervicalDiscHerniation','anteriorShoulderDislocation','slapLesion','lumbarSpinalStenosis','lumbarDiscHerniation','ankleSprain','meniscalInjury']){
    await evaluate(`window.reviewDisease=${JSON.stringify(disease)};$('pin-input').value='1234';checkPin();window.menuBeforeReviewDisease=JSON.stringify(S.menu);applyTemplate(reviewDisease);`);
    ...
    await evaluate("closeQR();closeTherapist();L={};persist();window.reviewDiseaseImportConfirmed=false;window.confirm=()=>{window.reviewDiseaseImportConfirmed=true;return true};manualImport(reviewDiseaseUrl);window.confirm=()=>true;");
    await check(disease+' import requires pain without making exercise records',"reviewDiseaseImportConfirmed&&S.patientId==='another_patient'&&Object.keys(L).length===0&&!!document.querySelector('.pain-gate')");
    await evaluate("saveVas(0)");
    await check(disease+' import shows a three-item carousel and patient restrictions',"document.querySelectorAll('#today-content .exercise-card').length===1&&$('today-content').textContent.includes('運動 1 / 3')&&JSON.stringify(S.menu.map(ex=>ex.exerciseKey))===JSON.stringify(reviewSharedKeys)&&S.menu.every((ex,i)=>ex.diseaseNote.includes(DISEASE_LIBRARY[reviewDisease].prescriptionNote)&&mediaFor(ex).image===reviewSharedImages[i])&&$('today-content').textContent.includes(patientCopy(DISEASE_LIBRARY[reviewDisease].prescriptionNote))");
  }
