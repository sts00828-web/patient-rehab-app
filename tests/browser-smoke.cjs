...
async function main(){
  ...
  for(const disease of ['rotatorCuffTear','cervicalDiscHerniation','anteriorShoulderDislocation','slapLesion','lumbarSpinalStenosis','lumbarDiscHerniation']){
    ...
    await evaluate("window.reviewSharedKeys=getDiseaseExerciseKeys(reviewDisease).filter(key=>['shoulder','lowback','knee','tennisElbow','cervicalSpondylosis','shoulderImpingement'].some(other=>getDiseaseExerciseKeys(other).includes(key))).slice(0,3).sort((a,b)=>ExerciseSelection.compare({exerciseKey:a},{exerciseKey:b}));window.reviewSharedImages=reviewSharedKeys.map(key=>EXERCISE_LIBRARY[key].image);");
