...
async function main(){
  ...
  await check('athlete catalog isolates sport candidates and requires individual load',"(()=>{const before=JSON.stringify(S);setCatalogAudience(true);const buttons=[...document.querySelectorAll('#exercise-catalog .tpl-card')].filter(b=>!b.hidden);if(buttons.length!==11||!buttons.every(b=>b.dataset.athlete==='true'))return false;applyTemplate('spondylolysisProtection');const good=document.querySelectorAll('#chooseTemplate .template-ex').length===4&&!$('pick-0').checked&&$('dose-0-load').value==='';closeModal('chooseTemplate');setCatalogAudience(false);return good&&JSON.stringify(S)===before&&!document.querySelector('#exercise-catalog [data-athlete=true]:not([hidden])')})()");
