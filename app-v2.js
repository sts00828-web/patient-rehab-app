...
function writableLog() {
  ...
  if(!log.menuSnapshot) {if(log.legacyUnknown) log.legacyOriginal=C.clone(log);log.menuSnapshot=C.menuAt(S,L,key,todayDow());delete log.legacyUnknown;}
...
function hasFlexiblePlan(){return S?.menu.some(ex=>ex.scheduleMode==='flexible')||S?.plans?.some(p=>p.menu.some(ex=>ex.scheduleMode==='flexible'));}
function percentText(c){if(hasFlexiblePlan())return c.unknown?'旧記録':c.done+'種目実施';return c.unknown?'旧記録':!c.total?'—':c.pct+'%';}
...
function renderToday(){
  ...
  h+=flexible?`<div class="card"><h2>今日の記録 ${summary.recorded}種目</h2><p>休む日も大切です。週の頻度に合わせて、ご自分のペースで。</p></div>`:`<div class="card"><div class="comp-bar"><div class="comp-pct record-count">${summary.recorded}<small> / ${items.length} 記録</small></div><div class="comp-d"><div class="comp-lbl">${before?'開始日は '+escapeHtml(S.startDate):!c.total?'今日は休養日です':`今日のできた種目 ${c.done} / ${c.total}`}</div><div class="bar-bg"><div class="bar-fg" style="width:${c.pct}%"></div></div></div></div><div class="hint">痛みで休んだことも、大切な記録です。</div></div>`;
  if(items.some(ex=>ex.scheduleMode==='flexible'))h+='<p class="hint">曜日は選びません。それぞれの週の頻度・休息に合わせて実施してください。毎日すべて行う必要はありません。</p>';
  for(const [i,ex] of items.entries()){
    const media=mediaFor(ex),st=log.status?.[ex.id]||(log.done?.[ex.id]?'done':'');
    h+=`<article class="card exercise-card"><div class="exercise-top"><div><span class="eyebrow">運動 ${i+1}</span><h2>${escapeHtml(ex.name)}</h2><div class="dose">${escapeHtml([ex.prescription?.repetitions,ex.prescription?.sets,ex.prescription?.frequency].filter(Boolean).join(' ／ ')||ex.params||'回数は担当者に確認してください')}</div></div>${media?`<img class="home-exercise-image" src="${media.imagePath||'assets/exercises/'+media.image}" onerror="this.hidden=true" alt="${escapeAttr(media.name)}" loading="lazy">`:''}</div>`;
    const restriction=[ex.clinicalV02?.constraints,ex.prescription?.load].find(value=>value?.trim());
    h+=`<div class="notice">指定側：${escapeHtml(ex.prescription?.side||'担当者に確認')}${restriction?` ／ 制限：${escapeHtml(restriction)}`:''}</div><details class="home-prescription"><summary>回数・頻度の詳細</summary>${prescriptionHtml(ex)}${exerciseNotices(ex,media)}</details>`;
  ...
  h+=flexible?`<div class="card record-summary" id="daily-record-summary" role="status"><p>${summary.recorded}種目を記録</p><p>${summary.recorded?'今日の記録は保存済みです。ここで閉じて大丈夫です。':'今日は休む日でも大丈夫です。'}</p><p>痛みとメモは任意です。</p></div>`:summary.html;
  if(!before){
  ...
  if(optionalRecordOpen&&document.querySelector('.optional-record'))document.querySelector('.optional-record').open=true;
}
...
function showExercise(i){
  const ex=todayExercises(todayDow())[i];if(!ex)return;showGuide(ex);
}
...
function closeModal(id){if(id==='clinicalShelf'&&typeof clinicalBatch!=='undefined')clinicalBatch=null;$(id)?.remove();syncOverlayState();}
...
function renderCalendar(){
  ...
  for(let d=1;d<=new Date(y,m+1,0).getDate();d++){
    ...
    h+=`<button class="cal-day ${key===todayKey()?'today':''} ${c.done?(hasFlexiblePlan()?'done':c.pct===100?'done':'partial'):''}" onclick="calSelect('${key}')" aria-label="${key} ${percentText(c)}">${d}${log.vas!==null?`<span class="cal-vas">${log.vas}</span>`:''}${c.done?'<span class="cal-dot full"></span>':''}</button>`;
  }
  h+='</div></div>';
  if(calSelectedKey){
...
function renderProgress(){
  ...
  const w=weekly(),flexible=hasFlexiblePlan(),weekDays=Array.from({length:7},(_,i)=>getLog(dk(addDays(new Date(),-i)))),recordDays=weekDays.filter(l=>Object.values(l.status||{}).some(v=>v&&v!=='cancelled')||Object.values(l.done||{}).some(Boolean)).length,exerciseDays=weekDays.filter(l=>Object.values(l.status||{}).some(v=>['done','partial'].includes(v))||Object.values(l.done||{}).some(Boolean)).length,points=[];for(let i=29;i>=0;i--){const d=addDays(new Date(),-i),log=getLog(dk(d));if(log.vas!==null)points.push({d,val:log.vas});}
  $('prog-content').innerHTML=`<div class="card"><h2>続けた記録</h2><div class="statgrid"><div class="statitem"><div class="statval">${flexible?recordDays:calcStreak()}</div><div class="statlbl">${flexible?'直近7日で記録した日数':'連続実施日数<br>休養日は除く'}</div></div><div class="statitem"><div class="statval">${flexible?exerciseDays:w.pct===null?'—':w.pct+'%'}</div><div class="statlbl">${flexible?'直近7日で運動した日数':'直近7日の達成率'}</div></div><div class="statitem"><div class="statval">${dayCount()}</div><div class="statlbl">開始からの日数</div></div></div><p class="hint">${flexible?'週の頻度に合わせて休みましょう。毎日の達成率では評価しません。':'開始前・休養日・当時のメニューが不明な日は計算から除きます。'}</p></div><div class="card"><h2>痛みの推移</h2><p class="hint">過去30日。未記録の日は点を表示しません。</p>${points.length?renderVasSvg(points):'<p class="empty">痛みの記録はまだありません。</p>'}</div>${S.nextVisit?`<div class="card"><h2>次回の予定</h2><p>${escapeHtml(S.nextVisit)}</p></div>`:''}<div class="card"><button class="btn btn-pri" onclick="showReport()">来院時に見せるレポート</button><button class="btn btn-out" onclick="exportPatientData()">自分の記録をバックアップ</button></div><div class="notice">記録はこの端末内に保存されています。スマホの変更やブラウザデータ削除の前にバックアップしてください。</div>`;
}
...
function menuTimingNotice(){
  const count=todayExercises(todayDow()).length;
  return `<div class="notice">患者画面の本日の対象：${count}種目（保存された実施頻度を確認してください）。${S.plans.some(p=>p.from>todayKey())?'保存した変更は明日から反映されます。<button type="button" id="apply-menu-today" class="btn btn-out" onclick="applyCurrentMenuToday()">保存したメニューを今日から反映</button>':''}</div>`;
}
function applyCurrentMenuToday(){
  if(!requireStaff())return;
  if(!confirm(`保存した${S.menu.length}種目のメニューを今日から反映しますか？\n曜日指定のない運動はいつでも表示します。週の頻度を守って実施してください。内容・回数が同じ種目の実施状況と、痛み・メモを引き継ぎます。変更前の実施記録はカレンダーに残します。`))return;
  try{C.applyMenuToday(S,L,todayKey(),todayDow());persist();renderTherapist();renderToday();if($('tab-cal').classList.contains('active'))renderCalendar();if($('tab-prog').classList.contains('active'))renderProgress();toast('保存したメニューを今日から反映しました');}
...
function initialPrescriptionDraft(ex,disease){
  ...
  if(DISEASE_LIBRARY[disease]?.audience==='athlete'&&disease!=='spondylolysisProtection'&&!standard.clinicianDefault){
    ...
    if(sport){Object.assign(result.prescription,sport.prescription);result.dows=[...sport.dows];result.defaultNote+=' 強化用の編集案です。週3日・実施日に1回を仮設定。練習負荷と休息を考慮して変更してください。';}
  }
...
function syncPrescriptionChoices(prefix){
  ...
  const week=$(prefix+'-frequency')?.value.match(/週([1-7])日/)?.[1];if(week&&$(prefix+'-weekly-days'))$(prefix+'-weekly-days').value=week;
...
function setWeeklyFrequency(prefix,value){
  if(!requireStaff())return;
  const input=$(prefix+'-frequency');
  input.value=input.value.replace(/・?週[1-7]日/g,'').replace(/・?毎日/g,'')+'・週'+value+'日';
  syncPrescriptionChoices(prefix);
}
function choosePrescription(prefix,key,index){
  if(!requireStaff()||!prescriptionChoices[key]?.[index])return;
  $(prefix+'-'+key).value=prescriptionChoices[key][index]+(key==='frequency'?'・週'+($(prefix+'-weekly-days')?.value||7)+'日':'');
  $(prefix+'-'+key).closest('details').open=false;syncPrescriptionChoices(prefix);
}
function setEveryday(prefix){if(!requireStaff())return;$(prefix+'-schedule-confirmed').checked=false;syncScheduleSummary(prefix);}
...
function athleteDoseButtons(ex,i,key){
  ...
  return `<div class="athlete-dose"><p>回数・時間・頻度の案を選ぶ（重量は個別設定）</p><button type="button" class="btn btn-out" onclick="applyAthleteDose(${i},2)">強化案：2セット</button><button type="button" class="btn btn-out" onclick="applyAthleteDose(${i},3)">発展案：3セット</button><p class="hint">重量・バンド抵抗、可動域、休息を負荷欄に指定。反復種目の発展案は抵抗を再評価し、回数を抑えて3セット。標準化された治療量ではありません。</p></div>`;
}
...
function doSaveTemplate(){
  ...
  T['custom_'+uid()]={name,icon:$('tpl-save-icon').value||'📋',desc:$('tpl-save-desc').value,menu:C.clone(S.menu)};T=cleanTemplates(T);persist();closeSaveTplModal();renderTherapist();toast('頻度とイラストも保存しました');
}
...
function archiveCurrent(){if(S&&(S.menu.length||Object.keys(L).length||['patientName','chartId','age','diagnosis','therapistName','affectedSide','painContext','consultContact','restartInstructions','nextVisit'].some(field=>S[field])||S.startDate!==todayKey())){if(archives.length>=500)throw Error('退避記録の上限です。バックアップを保存して整理してください。');archives.push({settings:C.clone(S),logs:C.clone(L),savedAt:new Date().toISOString()});}}
function newPatient(){
  if(!requireStaff())return;
  if($('new-patient-status')){$('new-patient-status').hidden=true;$('new-patient-status').textContent='';}
  if(storageBlocked||patientInputSaveFailed){toast('保存できていないため新規作成できません。入力を保存し直すか、再読み込みしてください。');return;}
  if(!confirm('現在の患者記録を退避し、別の患者を新規作成します。'))return;
  try{archiveCurrent();}catch(e){toast(e.message);return;}
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){for(const id of ['bugReportModal','guideModal','shareReviewModal','reportModal','chooseTemplate','exEditModal','archiveModal','tplManage']){const dialog=$(id);if(dialog){closeModal(id);break;}}}});
