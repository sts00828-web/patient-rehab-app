...
function manualImport(urlOverride){
  let txt=String(urlOverride??$('update-url')?.value??$('paste-url')?.value??'').trim();if(!txt){toast('URLを入力してください');return;}
  let part;try{part=collectSplitQrPart(txt);}catch(e){alert('読み込みできませんでした。\n'+e.message);return;}
  if(part&&!part.complete){alert(`分割QR ${part.count} / ${part.total} を読み取りました。続けて次のQRを読み取ってください。`);setTimeout(startQrScan,0);return;}
  if(part?.complete){txt=part.value;toast('分割QRをすべて読み取りました');}
